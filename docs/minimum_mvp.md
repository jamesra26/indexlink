...
13. 前端展示同一条链路：计划、三类信号、最终建议、双桶金额和 summary。
...
当前状态更准确地说是：投资计划与双桶执行层接近可演示；70% fundamental 可复用；20% trend 已可复用；AI 库层可复用；阿里云 API 接入、70/20/10 decision engine、最终 summary 和演示级前端仍是 MVP 缺口。
